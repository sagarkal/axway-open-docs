...
The cluster name is the value of the field als.clusterName in the agent override file hybrid-override.yaml generated by the Amplify CLI during Istio agent installation to your Kubernetes cluster. Choose a name for the http route name - ideally the same as the name for APIService or very similar to it. Make a note of this route name as we will need it in the next section.
